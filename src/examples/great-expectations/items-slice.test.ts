...
it('prefixes ids with "item-"', () => {
  ...
  expect(result).toEqual([expect.objectContaining({ name: 'iPhone', id: expect.stringMatching(/^item-/) })]);
});
...
it('supports toggling an item', () => {
  ...
  expect(result).toEqual([expect.objectContaining({...state[0], packed: true})]
  );
});
...
it('supports updating an item', () => {
  ...
  expect(result).toEqual([expect.objectContaining({...state[0], name: "Samsung Galaxy S23"})]);
});
